"""
Scrape the Russian Ministry of Defence English Telegram channel
(@mod_russia_en) via the public web preview at t.me/s/mod_russia_en.

Paginates backward through message history using the ?before=MSG_ID parameter.
Extracts message ID, date, text, embedded URLs. Saves to mod_telegram.csv.

Usage:
    python3 scrape_mod_telegram.py               # scrape all available
    python3 scrape_mod_telegram.py --max 5000     # scrape up to 5000 messages
    python3 scrape_mod_telegram.py --reset        # restart from latest
"""
...
CSV_PATH = BASE / "mod_telegram.csv"
PROGRESS_PATH = BASE / "mod_telegram_progress.json"
CHANNEL = "mod_russia_en"
...
def scrape(max_messages: int = 0) -> None:
    """Scrape MOD Telegram channel, paginating backward."""
...
def main():
    parser = argparse.ArgumentParser(description="Scrape MOD Russia Telegram channel")
    parser.add_argument(
