#!/usr/bin/env Rscript
# Smoothed figures using quarterly aggregation and rolling averages
...
# QUARTERLY aggregation (smoother)
quarterly_raw <- df %>%
...
p1 <- ggplot(quarterly, aes(x = year_quarter)) +
  ...
  labs(x = NULL, y = "Proportion of Speech Content (%)", color = NULL,
       title = sprintf("Regional Focus (Quarterly Averages, n = %d speeches, 2000-2025)", nrow(df)),
       subtitle = "Lemmatized corpus | Smoother time series") +
  theme_tufte() +
...
# --- PLOT 2: Ukraine Timeline (Quarterly) ---
...
p2 <- ggplot(quarterly, aes(x = year_quarter, y = ukraine_proportion)) +
  ...
  geom_line(linewidth = 1.8, color = "#f39c12", alpha = 0.9, na.rm = FALSE) +
  geom_point(size = 1.5, color = "#f39c12", alpha = 0.6, na.rm = TRUE) +
  ...
  labs(x = "Year", y = "Proportion of Speech Content (%)",
       ...
       subtitle = "Quarterly averages | Lemmatized corpus") +
  theme_tufte() +
  theme(plot.title = element_text(size = 13, face = "bold", hjust = 0.5),
        plot.subtitle = element_text(size = 10, hjust = 0.5),
        axis.text = element_text(size = 10), axis.title = element_text(size = 11),
...
# --- PLOT 3: Asia vs China (Quarterly) ---
p3 <- ggplot(quarterly, aes(x = year_quarter)) +
  ...
  geom_point(aes(y = asia_proportion, color = "Asia (All)"), size = 1.2, alpha = 0.5) +
  geom_point(aes(y = china_proportion, color = "China"), size = 1.2, alpha = 0.5) +
  scale_color_manual(values = c("Asia (All)" = "#e67e22", "China" = "#c0392b")) +
  labs(x = "Year", y = "Proportion of Speech Content (%)", color = NULL,
       title = "Asia & China Focus (Quarterly Averages, 2000-2025)") +
  theme_tufte() +
  theme(plot.title = element_text(size = 13, face = "bold", hjust = 0.5),
        axis.text = element_text(size = 10), axis.title = element_text(size = 11),
        legend.position = "top", legend.text = element_text(size = 10),
...
# Create complete sequence for recent period
...
# --- PLOT 5: Asian Countries Small Multiples (Quarterly) ---
...
p5 <- ggplot(countries_data, aes(x = year_quarter, y = pct)) +
  ...
  geom_point(size = 1.2, color = "#e67e22", alpha = 0.5) +
  facet_wrap(~country, scales = "fixed", ncol = 4) +
  labs(x = NULL, y = "% of Speech Content",
       title = sprintf("Asian Countries (Quarterly, 2021-present, n = %d speeches)", nrow(df_recent)),
...
cat("Quarterly aggregation provides smoother time series with fewer gaps.\n")
cat("Points added to show actual data density.\n")
